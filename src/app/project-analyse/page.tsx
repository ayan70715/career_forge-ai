...
export default function ProjectAnalyzerPage() {
  ...
  const exportPDF = async () => {
    ...
    pdf.text("Project Analysis Report", margin, y);
    y += 18;
  
    pdf.setFontSize(9);
    pdf.setTextColor(167, 139, 250);
    pdf.setFont("helvetica", "normal");
    pdf.text(`Generated by CareerForge AI · ${new Date().toLocaleDateString()}`, margin, y);
    y += 6;
  
    pdf.setDrawColor(217, 70, 239);
    pdf.setLineWidth(1);
    pdf.line(margin, y, pageWidth - margin, y);
    y += 16;
  
    // ── Overall Summary ──
    pdf.setFillColor(40, 15, 55);
    pdf.roundedRect(margin, y, contentWidth, 14, 4, 4, "F");
    pdf.setFontSize(10);
    pdf.setTextColor(217, 70, 239);
    pdf.setFont("helvetica", "bold");
    pdf.text("Overall Assessment", margin + 8, y + 9.5);
    y += 20;
  
    pdf.setFontSize(9);
    pdf.setTextColor(220, 230, 240);
    pdf.setFont("helvetica", "normal");
    const summaryLines = pdf.splitTextToSize(result.overallSummary, contentWidth);
    ...
    result.comparisons.forEach((comp) => {
      ...
      const scoreColor: [number, number, number] =
        comp.uniquenessScore >= 80 ? [34, 197, 94] :
        comp.uniquenessScore >= 50 ? [245, 158, 11] : [239, 68, 68];
  
      // Card background
      pdf.setFillColor(25, 18, 40);
      pdf.roundedRect(margin, y, contentWidth, 12, 3, 3, "F");
      pdf.setFontSize(11);
      pdf.setTextColor(241, 245, 249);
      pdf.setFont("helvetica", "bold");
      pdf.text(comp.resumeProjectName, margin + 8, y + 8.5);
  
      // Score badge
      pdf.setFillColor(...scoreColor);
      pdf.roundedRect(pageWidth - margin - 55, y + 1, 50, 11, 3, 3, "F");
      pdf.setFontSize(9);
      pdf.setTextColor(15, 10, 30);
      pdf.text(`${comp.uniquenessScore}/100`, pageWidth - margin - 42, y + 8.5);
      y += 16;
  
      pdf.setFontSize(8);
      pdf.setTextColor(148, 163, 184);
      pdf.setFont("helvetica", "italic");
      pdf.text(`vs. ${comp.matchedSimilarProject}`, margin + 8, y);
      y += 11;
  
      // Score bar
      pdf.setFillColor(50, 40, 70);
      pdf.roundedRect(margin, y, contentWidth, 5, 2, 2, "F");
      pdf.setFillColor(...scoreColor);
      pdf.roundedRect(margin, y, (contentWidth * comp.uniquenessScore) / 100, 5, 2, 2, "F");
      y += 10;
  
      pdf.setFont("helvetica", "italic");
      pdf.setTextColor(148, 163, 184);
      const scopeLines = pdf.splitTextToSize(comp.scopeComparison, contentWidth);
      ...
      const cols = [
        { title: "Feature Overlap", color: [245, 158, 11] as [number,number,number], items: comp.featureOverlap },
        { title: "Your Edge", color: [34, 197, 94] as [number,number,number], items: comp.differentiators },
        { title: "Suggestions", color: [217, 70, 239] as [number,number,number], items: comp.suggestions },
      ];
      ...
      cols.forEach(({ title, color, items }, ci) => {
        ...
        pdf.text(title, cx, cy);
        cy += 10;
        pdf.setFont("helvetica", "normal");
        pdf.setTextColor(203, 213, 225);
        pdf.setFontSize(7.5);
        items.slice(0, 4).forEach((item) => {
          const wrapped = pdf.splitTextToSize(`• ${item}`, colW);
      ...
      y += Math.max(
        comp.featureOverlap.length,
        comp.differentiators.length,
        comp.suggestions.length
      ) * 9 + 20;
    ...
    pdf.setTextColor(217, 70, 239);
    pdf.setFont("helvetica", "bold");
    pdf.text("Verified Similar Projects", margin, y);
    y += 14;
  
    result.similarProjects.forEach((proj) => {
      checkPageBreak(40);
      pdf.setFillColor(25, 18, 40);
      ...
      pdf.setTextColor(241, 245, 249);
      pdf.setFont("helvetica", "bold");
      pdf.text(proj.name, margin + 8, y + 7);
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(8);
      pdf.setTextColor(148, 163, 184);
      pdf.text(`⭐ ${proj.stars}  🍴 ${proj.forks}`, pageWidth - margin - 60, y + 7);
      y += 14;
      ...
      pdf.setTextColor(217, 70, 239);
      pdf.setFontSize(7.5);
      pdf.text(proj.url, margin, y);
      y += 14;
    });
  
    pdf.save("project-analysis.pdf");
  };
...
- 0-49: Very common, needs more differentiation`;
  ...
  return (
    ...
    >
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end">
                <Button onClick={exportPDF} variant="outline" className="gap-2 border-fuchsia-500/30 text-fuchsia-400 hover:bg-fuchsia-500/10">
      ...
      {/* Hidden PDF report */}
      {result && (
        <div ref={reportRef} style={{ position: "absolute", left: "-9999px", top: 0, width: "900px", padding: "48px", background: "linear-gradient(135deg, #0f0a1e 0%, #1a0a2e 50%, #0a1628 100%)", fontFamily: "sans-serif", color: "#fff" }}>
          {/* Header */}
          <div style={{ borderBottom: "2px solid #d946ef", paddingBottom: "24px", marginBottom: "32px" }}>
            <div style={{ background: "linear-gradient(90deg, #d946ef, #ec4899)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontSize: "28px", fontWeight: 800 }}>
              Project Analysis Report
            </div>
            <div style={{ color: "#a78bfa", fontSize: "13px", marginTop: "6px" }}>Generated by CareerForge AI · {new Date().toLocaleDateString()}</div>
          </div>
      
          {/* Overall Summary */}
          <div style={{ background: "rgba(217,70,239,0.1)", border: "1px solid rgba(217,70,239,0.3)", borderRadius: "12px", padding: "20px", marginBottom: "28px" }}>
            <div style={{ color: "#d946ef", fontWeight: 700, fontSize: "14px", marginBottom: "8px" }}>Overall Assessment</div>
            <div style={{ color: "#e2e8f0", fontSize: "13px", lineHeight: 1.7 }}>{result.overallSummary}</div>
            <div style={{ display: "flex", gap: "12px", marginTop: "14px", flexWrap: "wrap" }}>
              {[`${result.resumeProjects.length} Resume Projects`, `${result.similarProjects.length} Verified Matches`, `${result.comparisons.length} Comparisons`].map(label => (
                <span key={label} style={{ background: "rgba(217,70,239,0.15)", border: "1px solid rgba(217,70,239,0.3)", borderRadius: "20px", padding: "3px 12px", fontSize: "11px", color: "#d946ef" }}>{label}</span>
              ))}
            </div>
          </div>
          ...
          {/* Comparisons */}
          {result.comparisons.map((comp, i) => {
            const scoreColor = comp.uniquenessScore >= 80 ? "#22c55e" : comp.uniquenessScore >= 50 ? "#f59e0b" : "#ef4444";
            return (
              <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "22px", marginBottom: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "14px" }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "16px", color: "#f1f5f9" }}>{comp.resumeProjectName}</div>
                    <div style={{ color: "#94a3b8", fontSize: "12px", marginTop: "3px" }}>vs. {comp.matchedSimilarProject}</div>
                  </div>
                  <div style={{ background: scoreColor + "22", border: `1px solid ${scoreColor}55`, borderRadius: "8px", padding: "6px 14px", color: scoreColor, fontWeight: 700, fontSize: "13px" }}>
                    {comp.uniquenessScore}/100
                  </div>
                </div>
      
                {/* Score bar */}
                <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: "4px", height: "6px", marginBottom: "14px" }}>
                  <div style={{ background: scoreColor, width: `${comp.uniquenessScore}%`, height: "100%", borderRadius: "4px" }} />
                </div>
      
                <div style={{ color: "#94a3b8", fontSize: "12px", fontStyle: "italic", marginBottom: "16px" }}>{comp.scopeComparison}</div>
      
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
                  {[
                    { title: "Feature Overlap", color: "#f59e0b", items: comp.featureOverlap },
                    { title: "Your Edge", color: "#22c55e", items: comp.differentiators },
                    { title: "Suggestions", color: "#d946ef", items: comp.suggestions },
                  ].map(({ title, color, items }) => (
                    <div key={title} style={{ background: color + "11", border: `1px solid ${color}33`, borderRadius: "8px", padding: "12px" }}>
                      <div style={{ color, fontWeight: 700, fontSize: "11px", marginBottom: "8px" }}>{title}</div>
                      {items.map((item, j) => (
                        <div key={j} style={{ color: "#cbd5e1", fontSize: "11px", marginBottom: "4px" }}>• {item}</div>
                      ))}
                    </div>
                  ))}
                </div>
          ...
          <div style={{ marginTop: "8px" }}>
            <div style={{ color: "#d946ef", fontWeight: 700, fontSize: "15px", marginBottom: "14px" }}>Verified Similar Projects</div>
            {result.similarProjects.map((proj, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "10px", padding: "14px", marginBottom: "10px" }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div style={{ fontWeight: 600, fontSize: "13px" }}>{proj.name}</div>
                  <div style={{ color: "#94a3b8", fontSize: "11px" }}>⭐ {proj.stars} · 🍴 {proj.forks}</div>
                </div>
                <div style={{ color: "#94a3b8", fontSize: "11px", margin: "6px 0" }}>{proj.description}</div>
                <div style={{ color: "#d946ef", fontSize: "11px" }}>{proj.url}</div>
                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginTop: "8px" }}>
                  {proj.techStack.map(t => (
                    <span key={t} style={{ background: "rgba(217,70,239,0.1)", border: "1px solid rgba(217,70,239,0.25)", borderRadius: "12px", padding: "2px 8px", fontSize: "10px", color: "#d946ef" }}>{t}</span>
                  ))}
                </div>
              </div>
            ))}
